      ...
      ##########################################################################################################################################
       WHEN @malawi_ART_side_effects THEN
           SET @value = (SELECT name FROM concept_name WHERE concept_name_id = in_field_value_coded_name_id);
            ...
            IF in_field_voided = 0 THEN
              IF in_visit_id = 0 THEN
                INSERT INTO flat_table2 (patient_id, visit_date, malawi_ART_side_effects, malawi_ART_side_effects_enc_id) VALUES (in_patient_id, in_visit_date, @value, encounter_id);
              ELSE
                UPDATE flat_table2 SET malawi_ART_side_effects = @value, malawi_ART_side_effects_enc_id = encounter_id WHERE flat_table2.id = in_visit_id;
              END IF;
            ELSE
              UPDATE flat_table2 SET malawi_ART_side_effects = NULL, malawi_ART_side_effects_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
