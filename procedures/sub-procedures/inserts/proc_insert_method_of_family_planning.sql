...
BEGIN
    ...
    SET @family_planning_method_rythm_method = (SELECT concept_name.concept_id FROM concept_name LEFT OUTER JOIN concept ON concept.concept_id = concept_name.concept_id WHERE name = "Rhythm method" AND voided = 0 AND retired = 0 LIMIT 1);
    ...
    SET @family_planning_method_emergency_contraception = (SELECT concept_name.concept_id FROM concept_name LEFT OUTER JOIN concept ON concept.concept_id = concept_name.concept_id WHERE name = "Emergency contraception" AND voided = 0 AND retired = 0 LIMIT 1);
    ...
    CASE in_field_value_coded
        ...
        WHEN @family_planning_method_oral_contraceptive_pills THEN
            ...
            IF in_field_voided = 0 THEN
              ...
              END IF;
            ELSE
              UPDATE flat_table2 SET family_planning_method_oral_contraceptive_pills = NULL, family_planning_method_oral_contraceptive_pills_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
        ...
        WHEN @family_planning_method_depo_provera THEN
            ...
            IF in_field_voided = 0 THEN
              ...
              END IF;
            ELSE
              UPDATE flat_table2 SET family_planning_method_depo_provera = NULL, family_planning_method_depo_provera_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
        ...
        WHEN @family_planning_method_intrauterine_contraception THEN
            ...
            IF in_field_voided = 0 THEN
              ...
              END IF;
            ELSE
              UPDATE flat_table2 SET family_planning_method_intrauterine_contraception = NULL, family_planning_method_intrauterine_contraception_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
        ...
        WHEN @family_planning_method_contraceptive_implant THEN
            ...
            IF in_field_voided = 0 THEN
              ...
              END IF;
            ELSE
              UPDATE flat_table2 SET family_planning_method_contraceptive_implant = NULL, family_planning_method_contraceptive_implant_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
        ...
        WHEN @family_planning_method_male_condoms THEN
            ...
            IF in_field_voided = 0 THEN
              ...
              END IF;
            ELSE
              UPDATE flat_table2 SET family_planning_method_male_condoms = NULL, family_planning_method_male_condoms_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
        ...
        WHEN @family_planning_method_female_condoms THEN
            ...
            IF in_field_voided = 0 THEN
              ...
              END IF;
            ELSE
              UPDATE flat_table2 SET family_planning_method_female_condoms = NULL, family_planning_method_female_condoms_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
        
        WHEN @family_planning_metho_rythm_method THEN
            ...
            IF in_field_voided = 0 THEN
              IF in_visit_id = 0 THEN
                  ...
                  INSERT INTO flat_table2 (patient_id, visit_date, family_planning_method_rythm_method, family_planning_method_rythm_method_enc_id) VALUES (in_patient_id, in_visit_date, @value, encounter_id);
              
              ELSE 
              
                  UPDATE flat_table2 SET family_planning_method_rythm_method = @value, family_planning_method_rythm_method_enc_id = encounter_id WHERE flat_table2.id = in_visit_id;
                  
              END IF;
            ELSE
              UPDATE flat_table2 SET family_planning_method_rythm_method = NULL, family_planning_method_rythm_method_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
        ...
        WHEN @family_planning_method_withdrawal THEN
            ...
            IF in_field_voided = 0 THEN
              ...
              END IF;
            ELSE
              UPDATE flat_table2 SET family_planning_method_withdrawal = NULL, family_planning_method_withdrawal_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
        ...
        WHEN @family_planning_method_abstinence THEN
            ...
            IF in_field_voided = 0 THEN            
              ...
              END IF;
            ELSE
              UPDATE flat_table2 SET family_planning_method_abstinence = NULL, family_planning_method_abstinence_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
        ...
        WHEN @family_planning_method_tubal_ligation THEN
            ...
            IF in_field_voided = 0 THEN           
              ...
              END IF;
            ELSE
              UPDATE flat_table2 SET family_planning_method_tubal_ligation = NULL, family_planning_method_tubal_ligation_enc_id = NULL WHERE flat_table2.id = in_visit_id;
            END IF;
        ...
        WHEN @family_planning_method_emergency_contraception THEN
            ...
            IF in_field_voided = 0 THEN
              IF in_visit_id = 0 THEN
                  ...
                  INSERT INTO flat_table2 (patient_id, visit_date, family_planning_method_emergency_contraception, family_planning_method_emergency_contraception_enc_id) VALUES (in_patient_id, in_visit_date, @value, encounter_id);
              
              ELSE 
              
                  UPDATE flat_table2 SET family_planning_method_emergency_contraception = @value, family_planning_method_emergency_contraception_enc_id = encounter_id WHERE flat_table2.id = in_visit_id;
