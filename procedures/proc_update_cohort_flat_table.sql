...
CREATE PROCEDURE `proc_update_cohort_flat_table`(
	...
	BEGIN
		...
		IF DATE(in_extrapulmonary_tuberculosis_v_date) > DATE(@var_extrapulmonary_tuberculosis_v_date)  THEN 
			...
			UPDATE flat_cohort_table SET extrapulmonary_tuberculosis extrapulmonary_tuberculosis_v_date WHERE patient_id = in_patient_id
		...
		IF DATE(in_pulmonary_tuberculosis_last_2_years) > DATE(@var_pulmonary_tuberculosis_last_2_years_v_date) THEN
