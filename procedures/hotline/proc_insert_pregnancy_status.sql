  ...
  IF @already_exist = 0 THEN
    ...
    ELSE
      IF in_field_voided = 0 THEN
        UPDATE patient_visits SET pregnancy_status = in_field_text, pregnancy_status_enc_id = encounter_id WHERE patient_visits.id = in_visit_id;
      END IF;
    END IF;
  ELSE
    IF in_visit_id = 0 THEN
      UPDATE patient_visits SET visit_date = in_visit_date, pregnancy_status = in_field_text, pregnancy_status_enc_id = encounter_id
      WHERE patient_id = in_patient_id;
    ELSE
      IF in_field_voided = 0 THEN
        UPDATE patient_visits SET visit_date = in_visit_date, pregnancy_status = in_field_text, pregnancy_status_enc_id = encounter_id WHERE patient_visits.id = in_visit_id;
      END IF;
    END IF;
  END IF;
