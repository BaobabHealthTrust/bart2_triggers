...
CREATE PROCEDURE`proc_insert_patient_identifier`(
	...
	BEGIN
		...
		CASE identifier_type
			...
			WHEN @cellphone THEN
				UPDATE flat_table1 SET cellphone_number = identifier WHERE patient_id = pat_id;
			WHEN @home_number THEN
				UPDATE flat_table1 SET home_phone_number = identifier WHERE patient_id = pat_id;	
			WHEN @office_number THEN
	 			UPDATE flat_table1 SET office_phone_number = identifier WHERE patient_id = pat_id;		
