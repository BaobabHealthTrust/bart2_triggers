DELIMITER $$
DROP TRIGGER IF EXISTS `observation_after_update2`$$
CREATE TRIGGER `observation_after_update2` AFTER UPDATE
ON `obs`
FOR EACH ROW
BEGIN
  ...
  SET @encounter_type = COALESCE(( SELECT encounter_type FROM encounter
                           ...
                           AND voided = 0 AND encounter_type NOT IN (5, 40, 71, 72, 73, 90, 111, 150, 151)), 0);
